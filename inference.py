"""
inference.py - FINAL (Proxy-safe, no silent fallback)
"""
...
# ENV VARIABLES (provided by evaluator)
...
Choose your next action from available actions."""
    ...
    # fallback only after successful LLM call (allowed)
    for a in available:
...
def run_episode(client: OpenAI, task: str) -> None:
    ...
    log_start(task=task, env="fraud_detect_env", model=MODEL_NAME)
